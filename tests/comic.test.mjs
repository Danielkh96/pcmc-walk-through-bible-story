...
test("comic query navigation clamps bounds and rejects invalid page numbers", () => {
  ...
  assert.equal(getComicPageIndex("10", 10), 9);
  assert.equal(getComicPageIndex("12", 10), 9);
  assert.equal(getComicPageIndex("99", 10), 9);
});
...
test("service worker caches comic navigation separately from the homepage", async () => {
  ...
  runInNewContext(script, {
    ...
    URL, Response,
    fetch: async (request) => { if (offline) throw new Error("offline"); return new Response(request.url.includes("episode-04") ? "chapter four" : request.url.includes("episode-03") ? "chapter three" : request.url.includes("episode-02") ? "chapter two" : "updated comic"); },
    caches: {
  ...
  assert.equal(await (await navigate("/comic/read?chapter=episode-03&page=1")).text(), "chapter three");
  assert.equal(await (await navigate("/comic/read?chapter=episode-04&page=1")).text(), "chapter four");
  offline = true;
  ...
  assert.equal(await (await navigate("/comic/read?chapter=episode-03&page=10")).text(), "chapter three");
  assert.equal(await (await navigate("/comic/read?chapter=episode-04&page=8")).text(), "chapter four");
...
test("public comic assets exclude deletable local archives and obsolete editions", async () => {
  ...
  assert.deepEqual(directories.sort(), ["adam-to-noah-v1", "adam-to-noah-v1-en", "book-v1", "brothers-v1", "brothers-v1-en", "creation-mystery-v4", "creation-mystery-v4-en", "eden-v1", "eden-v1-en", "generations-v1", "generations-v1-en", "noahs-time-v1", "noahs-time-v1-en", "temptation-choice-v1", "temptation-choice-v1-en"]);
...
test("Eden has ten complete pages and a separate reading edition", async () => {
  const chapter = catalog.chapters.find(c => c.id === "eden");
  assert.ok(chapter);
  assert.equal(chapter.title, "伊甸园");
  assert.equal(chapter.edition, "eden-v1");
  assert.equal(chapter.number, 2);
  assert.equal(chapter.reference, "创世记 2:4–25");
  assert.deepEqual(chapter.newCharacterIds, ["adam", "woman"]);
  const pages = JSON.parse(await readFile(new URL("../app/comic/eden-pages.json", import.meta.url), "utf8"));
  ...
  const panels = [...storyboard.matchAll(/^## 第(\d+)页[^\n]*（(\d+)格）/gm)];
  assert.deepEqual(panels.map(p => Number(p[2])), [4,4,5,4,4,4,5,5,4,4]);
  assert.equal(panels.reduce((sum,p) => sum + Number(p[2]), 0), 43);
  const edition = { id: chapter.id, edition: chapter.edition, pages };
  assert.equal(savedComicPage(JSON.stringify({ chapter: "creation", edition: "creation-mystery-v4", page: 7 }), edition), 1);
  assert.equal(savedComicPage(JSON.stringify({ chapter: "eden", edition: "eden-v1", page: 7 }), edition), 7);
});

test("Temptation has fourteen finished pages, sixty-two panels and isolated reading progress", async () => {
  const chapter = catalog.chapters.find(c => c.id === "temptation");
  assert.equal(chapter.title, "试探与选择");
  assert.equal(chapter.number, 3);
  assert.equal(chapter.edition, "temptation-choice-v1");
  assert.equal(chapter.reference, "创世记 3:1–24");
  assert.deepEqual(chapter.newCharacterIds, ["serpent"]);
  const pages = JSON.parse(await readFile(new URL("../app/comic/temptation-pages.json", import.meta.url), "utf8"));
  ...
  const panels = [...storyboard.matchAll(/^### 第(\d+)页[^\n]*（(\d+)格）/gm)];
  assert.deepEqual(panels.map(p => Number(p[2])), [4,5,4,5,5,4,5,5,4,4,5,4,4,4]);
  assert.equal(panels.reduce((sum,p) => sum + Number(p[2]), 0), 62);
  const edition = { id: chapter.id, edition: chapter.edition, pages };
...
test("Generations publishes nine localized pages in Chinese and English", async () => {
  ...
  for (const page of pages) {
    ...
    assert.equal(page.imageEn, `/comics/generations-v1-en/page-${String(page.id).padStart(2, "0")}.png`);
    const bytes = await readFile(new URL("../public" + page.image, import.meta.url));
    assert.equal(bytes.subarray(0, 8).toString("hex"), "89504e470d0a1a0a");
    assert.equal(bytes.readUInt32BE(16), 1024);
    assert.equal(bytes.readUInt32BE(20), 1536);
