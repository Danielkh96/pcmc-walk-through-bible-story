...
test('Chapter six fills the gap with ten complete Chinese and English pages',async()=>{
 ...
 assert.deepEqual(c.availableLanguages,['zh','en']);
 ...
 for(const p of pages){
  ...
  assert.equal(p.imageEn,p.image.replace('adam-to-noah-v1/','adam-to-noah-v1-en/'));
  const english=await read('public'+p.imageEn);
  assert.equal(english.readUInt32BE(16),1024);assert.equal(english.readUInt32BE(20),1536);
  assert.notDeepEqual(english,await read('public'+p.image));
